...
class Security extends DBMaster{
    ...
    function sso() {
        ...
        if (
            ...
            if (empty($usrInfo)){
                ...
                $this->auditData(0,self::SYSTEM_LOGIN_BADUSER,['user' => $data->email],null);
            }else{
                if ( $usrInfo['ext_id'] == $data->extid ){
                    if ($usrInfo['active'] == 0){
                        $validationStatus = [
                                    'result' => 0,
                        ...
                        $this->saveData('system_login',['lastlogin' => date('Y-m-d H:i:s', strtotime('now'))],$usrInfo['id'], true);
                        $this->saveData('users',['image' => $data->image],$usrInfo['id'], true);
                    ...
                    $this->auditData(0,self::SYSTEM_LOGIN_BADPASS,['user' => $data->email],null);
                }
            }
    ...
    function register() {
        ...
        if (
            ...
            $this->auditData(1,DBMaster::SYSTEM_LOGIN_BADREQUEST,$data,null);
            return ['result' => 0, 'errorcode' => DBMaster::SYSTEM_LOGIN_BADREQUEST,'message' => 'Bad Request'];
        }
        ...
        if (empty($usrExists)){
                ...
                if (is_array($createResult)){
                    ...
                    $this->auditData(2,self::SYSTEM_USER_USERCREATED,['id' => $usrId, 'user' => $data->email],null);
                    return $this->generateToken(['data' => ['id' => $usrId, 'ext_id' => null]]);
                }
            
        } else {
            $this->auditData(2,self::SYSTEM_USER_USEREXISTS,['user' => $data->email],null);
            return [
                'result' => 0,                        
                'errorcode' => self::SYSTEM_USER_USEREXISTS,
                'message' => 'La casilla de correo ya está registrada'
            ];
        }
        
    }

    function createUser($user, $password, $active){
        if (!$this->tableExists('system_login')){
			return ['result' => 0, 'message' => 'Table not exists'];
        }
        ...
        if (empty($usrInfo)){
            ...
            $usrId = $this->saveData('system_login',['usr' => $user, 'pass' => password_hash($password, PASSWORD_DEFAULT), 'active' => $active, 'activationcode' => $recoveryCode, 'activationcodeexp' => date('Y-m-d H:i:s', strtotime('+1 day'))],null, true); 
            $result = [
                        'result' => 1,
    ...
    function activateUser($user, $code){
        ...
        if (!empty($usrInfo)){
            if ($code != $usrInfo['activationcode']){
               ...
               $this->saveData('system_login',['active' => 1],$usrInfo['id'], true); 
               $result = [
                        'result' => 1,
                        'data' => $usrInfo['id']
                    ];
